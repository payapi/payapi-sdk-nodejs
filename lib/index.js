(function () {
  ...
  var log = require("./log")("validation");
  ...
  module.exports = function PayapiClient(params) {
    function rejectUnsafeEnvironmentVariable(params) {
      return new Promise(function(resolve, reject) {
        if(process.env.NODE_TLS_REJECT_UNAUTHORIZED && ["development", "test"].indexOf(process.env.NODE_ENV) < 0) {
          return reject(new Error("NODE_TLS_REJECT_UNAUTHORIZED is not allowed in environment '" +
    ...
    function sanitizeAndExtractBinAndLast4(apiKeyAndToken) {
      ...
      if(paymentObject && paymentObject.payment && paymentObject.payment.creditCardNumber &&
        params.optionalFields.indexOf("payment") < 0 && params.optionalFields.indexOf("payment.creditCardNumber") < 0) {
    ...
    function validatePayload(params) {
      return new Promise(function(resolve, reject) {
        ...
        if (validationErrors.length > 0 && params.fillableFields) {
          validationErrors = validationErrors.filter(function(error) {
            return !(error.translationKey && error.translationKey.includes("invalid.") &&
              params.fillableFields.indexOf(error.translationKey.substr(8)) > -1);
          });
        }
    ...
    function stopMoment(timerName, startMoment, noisy, logger) {
      if(noisy === true) {
        ...
        if(logger.log && logger.log.constructor && logger.log.constructor.name === "Function") {
          ...
          log.warn("The logger you have provided does not have a log('message') interface");
        }
      }
    }
    ...
    function doRestCall(params) {
      return new Promise(function(resolve, reject) {
        ...
        switch(process.env.NODE_ENV) {
          case "production":
            uri = params.uri || "https://input.payapi.io/v1/api/authorized/payments";
            break;
          case "staging":
            uri = params.uri || "https://staging-input.payapi.io/v1/api/authorized/payments";
            break;
          default:
            uri = params.uri || "http://localhost:3000/v1/api/authorized/payments";
        }
        rp({
          method: "POST",
          uri: uri,
    ...
    function handleError(err) {
      return new Promise(function(resolve, reject) {
        log.error("Payapi Client Error:" + JSON.stringify(err, null, 2));
        reject(err);
      });
    }
      ...
      return new Promise(function(resolve, reject) {
        if(params.payload.products && params.payload.products.length > 0) {
          for(var i = 0; i < params.payload.products.length; i++) {
            ...
            params.payload.products[i].vatInCents = Number(params.payload.products[i].vatInCents);
          }
        }
        if(params.payload && params.payload.order) {
          ...
          params.payload.order.vatInCents = Number(params.payload.order.vatInCents);
        }
        resolve(params);
      });
    }
    ...
    return {
      ...
      decodePaymentToken: function() {
        ...
        if(typeof paymentObject !== "object") {
          log.debug("JSON object not valid: %O", paymentObject);
          throw [{
