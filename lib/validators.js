(function () {
  ...
  function sanitizeForValidation(value) {
    value = value ? validator.stripLow(value).replace(/ /g, "") : value;
    return value;
  }
  ...
  exports.InputDataValidator = function(params) {
    ...
    function validatePayment(payment) {
      ...
      if(!validator.isAlpha(payment.paymentMethod)) {
        validationErrors.push({
      ...
      var ccv = sanitizeForValidation(payment.ccv);
      if(!ccv ||
    ...
    function validateConsumer(consumer) {
      ...
      if(!country ||
          country.length < 2 ||
          country.length > 53 ||
          country.match(BLACKLISTED_CHARACTERS)) {
        validationErrors.push({
      ...
      var locale = sanitizeForValidation(consumer.locale) || "en-US";
      if(locale.length < 5 ||
         locale.length > 10 ||
    ...
    function validateOrder(order) {
      ...
      if(order.referenceId && order.referenceId.length > 255) {
        validationErrors.push({
      ...
      if(order.referenceId && !validator.isAlphanumeric(order.referenceId)) {
        validationErrors.push({
          value: order.referenceId,
          message: "Invalid order reference ID",
          translationKey: "invalid.order.referenceId",
