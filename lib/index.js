(function () {
  ...
  module.exports = function PayapiClient(params) {
    ...
    function encodePayloadExtraComponents(payload) {
      console.log('encodePayloadUriComponents()')
        console.log(payload.extra);
      if(payload.extra) {
        var newExtra = {};
        for(var key in payload.extra) {
          if(payload.extra.hasOwnProperty(key) && payload.extra[key]) {
            newExtra[encodeURIComponent(key)] = encodeURIComponent(payload.extra[key]);
          }
        }
        payload.extra = newExtra;
      }
