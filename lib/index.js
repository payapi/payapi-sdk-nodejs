  ...
  module.exports = function PayapiClient(params) {
    ...
    function encodePayloadUriComponents(payload) {
      ...
      if(payload.returnUrls && payload.returnUrls.success) {
        payload.callbacks.success = Url.parse(payload.returnUrls.success).href;
      }
      if(payload.returnUrls && payload.returnUrls.cancel) {
        payload.callbacks.cancel = Url.parse(payload.returnUrls.cancel).href;
      }
      if(payload.returnUrls && payload.returnUrls.failed) {
        payload.callbacks.failed = Url.parse(payload.returnUrls.failed).href;
      }

      // ToS
      if(payload.order.tosUrl) {
    ...
    function validatePayload(params) {
      return new Promise(function(resolve, reject) {
        var payload = encodePayloadUriComponents({
