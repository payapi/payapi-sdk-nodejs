      ...
      function validateReferenceId() {
        if(isOptional("referenceId") && !order.referenceId) {
          ...
          if(order.referenceId) {
            if(order.referenceId.match(BLACKLISTED_CHARACTERS)) {
              validationErrors.push({
                value: "Order referenceId is not URL encoded",
                message: "Invalid order referenceId",
                elementName: "order[referenceId]",
                translationKey: "invalid.order.referenceId",
              });
            } else if(order.referenceId.length > 255) {
              validationErrors.push({
