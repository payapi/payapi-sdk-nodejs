  ...
  var ConsumerValidator = (function() {
    var ConsumerValidator = function(params) {
      ...
      function validateCity() {
        ...
        if(!consumer.city && isOptional("city")) {
          return;
        } else {
          if(consumer.city) {
            if(!isValidCity(city)) {
              ...
              validationErrors.push({
                value: "Consumer city is not URL encoded",
                message: "Invalid consumer city",
                elementName: "consumer[city]",
                translationKey: "invalid.consumer.city",
              });
      ...
      function validateStateOrProvince() {
        ...
        if (!consumer.stateOrProvince && isOptional("stateOrProvince")) {
          return;
        } else {
          if (consumer.stateOrProvince) {
            if (stateOrProvince.length < 2 || stateOrProvince.length > 52) {
              ...
              validationErrors.push({
                value: "Consumer state or province is not URL encoded",
                message: "Invalid consumer state or province",
                elementName: "consumer[stateOrProvince]",
                translationKey: "invalid.consumer.stateOrProvince",
              });
      ...
      function validateCountryCode() {
        ...
        if (!consumer.countryCode && isOptional("countryCode")) {
          return;
        } else {
          if (consumer.countryCode) {
      ...
      function isValidPhoneNumber(phone) {
        try {
          ...
          var mobilePhoneNumber = String(phone).indexOf("+") >= 0 ? phone : "+" + phone;
      ...
      function validatePhoneNumber() {
        ...
        if (!consumer.mobilePhoneNumber && isOptional("mobilePhoneNumber")) {
          return;
        } else {
          if(!consumer.mobilePhoneNumber) {
            validationErrors.push({
            ...
            if (mobilePhoneNumber.match(BLACKLISTED_CHARACTERS)) {
              validationErrors.push({
                value: "Consumer mobile phone number is not URL encoded",
                message: "Invalid consumer mobile phone number",
                elementName: "consumer[mobilePhoneNumber]",
                translationKey: "invalid.consumer.mobilePhoneNumber",
              });
            } else if(!isValidPhoneNumber(mobilePhoneNumber)) {
              validationErrors.push({
      ...
      function validateEmail() {
        ...
        if(!consumer.email && isOptional("email")) {
          return;
        } else {
          if(consumer.email) {
            if(email.match(BLACKLISTED_CHARACTERS)) {
              validationErrors.push({
                message: "Invalid consumer email",
                translationKey: "invalid.consumer.email",
                elementName: "consumer[email]",
                value: "Consumer email is not URL encoded"
              });
      ...
      function validateConsumerId() {
        ...
        if (!consumer.consumerId && isOptional("consumerId")) {
          return;
        } else {
          if (consumer.consumerId) {
            if (!isValidName(consumerId)) {
              validationErrors.push({
                value: "Consumer consumerId must be between 2 and 52 characters",
                message: "Invalid consumer consumerId",
                elementName: "consumer[consumerId]",
                translationKey: "invalid.consumer.consumerId",
              });
            } else if (consumerId.match(BLACKLISTED_CHARACTERS)) {
              validationErrors.push({
                value: "Consumer consumerId is not URL encoded",
                message: "Invalid consumer consumerId",
                elementName: "consumer[consumerId]",
                translationKey: "invalid.consumer.consumerId",
              });
