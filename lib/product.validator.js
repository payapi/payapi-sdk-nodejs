      ...
      function validateExtraData() {
        if(!product.extraData && isOptional("extraData")) {
          ...
          if(product.extraData) {
            for(var key in product.extraData) {
              if(key.match(BLACKLISTED_CHARACTERS) || product.extraData[key].match(BLACKLISTED_CHARACTERS)) {
                validationErrors.push({
                  value: "Product extraData is not URL encoded. Offending key or value: " + encodeURIComponent(key),
